...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer> neighbor = new LinkedList<>();
		for(int i=0;i<adjMatrix[v].length;i++) { //each column of row v
			for(int j=0;j<adjMatrix[v][i];j++)	//if adjMatrix[v][j]>0 , it executes the loop.
				for(Integer each : getNeighbors(i)) {
					neighbor.add(each);
				}		
		}
		return neighbor;
	...
	public static void main(String[] args) {
		GraphAdjMatrix simpleTest = new GraphAdjMatrix();
		GraphLoader.loadGraph("/Users/yunsangil/Documents/computing/JAVA/UCSDGraphs/data/graders/mod1/graph1.txt", simpleTest);
		System.out.println(simpleTest);
		
		System.out.println("2hop : " + simpleTest.getDistance2(4));
	}
